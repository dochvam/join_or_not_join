...
runtime1.1 <- bind_rows(runtime_results) %>% 
  select(type, scenario, iter, vaule) %>% 
  pivot_wider(names_from = type, values_from = vaule) %>% 
  mutate(improvement = joint - one) %>% 
  group_by(scenario) %>% 
  summarize(mean = mean(improvement), ub = quantile(improvement, 0.25),
            lb = quantile(improvement, 0.75)) %>% 
  left_join(specs_df, by = "scenario")
...
panelA <- ggplot(summary1.1,
                 aes(nPA, mean, ymax = ub, ymin = lb, col = as.factor(S),
                     group = as.factor(S))) +
  geom_pointrange(position = position_dodge(width = 5)) +
  geom_line(position = position_dodge(width = 5)) +
  theme_minimal() +
...
panelC <- ggplot(summary1.2,
                 aes(alpha0, mean, ymax = ub, ymin = lb, col = as.factor(theta0),
...
sim_results_fig <- arrangeGrob(panelA, panelB, panelC, panelD, panelE, panelF,
                               nrow = 3)
ggsave("sim_results_main_S2.jpg", sim_results_fig, width = 9, height = 10, dpi = 300)
